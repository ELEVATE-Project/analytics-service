...
Maps batch data to approved themes at a custom threshold (e.g. $70\%$), then clusters any leftover unmatched statements into new draft themes using TriTopic and LLM.
...
Use this when you want to process the statements from the database that are currently tagged as `"Others"` (meaning they couldn't be matched to any approved theme).
...
> ⚠️ **Note:** The batch tool threshold is flexible and user-controlled.
