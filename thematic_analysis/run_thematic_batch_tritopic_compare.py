...
# Also support running from Desktop by checking standard Documents path for app
repo_dir = Path("/Users/user/Documents/AI/analytics-arch/analytics_service")
if repo_dir.is_dir() and str(repo_dir) not in sys.path:
    sys.path.insert(0, str(repo_dir))
    if not root_dir:
        root_dir = repo_dir

# Load local .env from the script's folder first
...
if local_env.is_file():
    try:
        ...
        with open(local_env, mode="r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
...
logger = logging.getLogger("thematic_batch_tritopic_compare")
...
async def main_async():
    parser = argparse.ArgumentParser(description="Standalone comparison-based batch thematic analysis runner using TriTopic.")
    parser.add_argument(
        "--csv",
    ...
    database_url = os.environ.get("DATABASE_URL", "postgresql://postgres:postgres@localhost:5432/analytics_db")
    ...
    try:
        ...
        # 5. Cluster all objectives using TriTopic
        logger.info("Clustering ALL objectives locally using TriTopic...")
        tritopic_cfg = TriTopicConfig(
            use_dim_reduction=len(raw_objectives) >= 15,
        ...
        with clean_tritopic_print():
            tritopic_model.fit(objectives_list, embeddings=objectives_embeddings)
        
        labels = tritopic_model.labels_
        ...
        # 6. Compare clusters to existing themes using centroids
        for local_id in unique_labels:
            cluster_indices = np.where(labels == local_id)[0]
            cluster_objectives = [raw_objectives[idx] for idx in cluster_indices]
            cluster_texts = [item["objective"] for item in cluster_objectives]
            cluster_embs = objectives_embeddings[cluster_indices]

            # Compute the cluster centroid
            cluster_centroid = np.mean(cluster_embs, axis=0).reshape(1, -1)

            # Find the best matching approved theme for the centroid
            best_theme_id = None
            best_score = -1.0
            for theme in approved_themes:
                theme_id = str(theme["id"])
                ...
                # Compute sims between centroid and all vectors of this theme
                sims = cosine_similarity(cluster_centroid, theme_vectors)[0]
                max_sim = np.max(sims)
            ...
            if best_score >= CROSS_REF_SIMILARITY_THRESHOLD:
                # Map the entire cluster to this approved theme
                approved_theme_name = next((t["name"] for t in approved_themes if str(t["id"]) == best_theme_id), f"Approved Theme {best_theme_id}")
                logger.info(f"Cluster {local_id + 1} maps to Approved Theme '{approved_theme_name}' with similarity {best_score:.4f}.")
                for idx, item in zip(cluster_indices, cluster_objectives):
                    obj_emb = objectives_embeddings[idx].reshape(1, -1)
                    theme_vectors = approved_themes_vectors[best_theme_id]
                    sims_ind = cosine_similarity(obj_emb, theme_vectors)[0]
                    max_sim_ind = np.max(sims_ind)
                ...
                for t in tritopic_model.topics_:
                    if t.topic_id == local_id:
                        t.label = approved_theme_name
            else:
                # Create a new Draft theme from this cluster
                logger.info(f"Cluster {local_id + 1} does not match existing themes (best similarity {best_score:.4f}). Creating new Draft theme...")
                ...
                member_similarities = cosine_similarity(cluster_embs, cluster_centroid.reshape(1, -1))
        ...
        if draft_themes:
            ...
            try:
                fig_topics = tritopic_model.visualize_topics(width=1000)
                # Increase left margin to prevent cut-off y-axis text
                fig_topics.update_layout(
                    margin=dict(l=220, r=50, t=80, b=50),
                )
                fig_topics.write_html(str(viz_dir / "topics_keywords.html"))
                logger.info(f"Saved Topics Keywords to {viz_dir / 'topics_keywords.html'}")
            except Exception as e:
                logger.error(f"Failed to generate Topics Keywords: {e}")

            # 3. Hierarchy Dendrogram
            try:
                fig_hier = tritopic_model.visualize_hierarchy(width=900, height=550)
                ...
                logger.info(f"Saved Hierarchy Tree to {viz_dir / 'hierarchy_tree.html'}")
            except Exception as e:
            ...
            try:
                ...
                fig_sim.update_layout(
                    margin=dict(l=220, r=50, t=80, b=220),
                )
        ...
        # 7. Map individual outlier objectives (label -1) to approved themes if they exceed similarity threshold
        outlier_indices = np.where(labels == -1)[0]
        logger.info(f"Processing {len(outlier_indices)} outliers individually...")
        outliers_mapped_count = 0
        for idx in outlier_indices:
            item = raw_objectives[idx]
            obj_emb = objectives_embeddings[idx].reshape(1, -1)
            ...
            for theme in approved_themes:
                theme_id = str(theme["id"])
                theme_vectors = approved_themes_vectors[theme_id]
                sims = cosine_similarity(obj_emb, theme_vectors)[0]
                max_sim = np.max(sims)
                if max_sim > best_score:
                    best_score = max_sim
                    best_theme_id = theme_id
            ...
            if best_score >= CROSS_REF_SIMILARITY_THRESHOLD:
                ...
                outliers_mapped_count += 1

        logger.info(f"Successfully mapped {outliers_mapped_count} outliers to approved themes.")

        # 8. Save results back to Postgres database
        await save_mappings_to_db(conn, mapped_themes_data, draft_themes, all_statements_dict)

        # 9. Print overview statistics
        mapped_count = sum(len(v) for v in mapped_themes_data.values())
        unmapped_count = len(raw_objectives) - mapped_count
        ...
        # 10. Generate PM review CSV automatically
