...
logger = logging.getLogger("thematic_batch")
...
CLUSTER_DISTANCE_THRESHOLD = 0.45      # Agglomerative clustering threshold (cosine distance = 1 - similarity)
...
async def main_async():
    parser = argparse.ArgumentParser(description="Standalone stand-alone batch thematic analysis runner.")
    parser.add_argument(
        "--csv",
    ...
    parser.add_argument(
        "--output",
        default="hierarchical_theme_pm_review.csv",
    ...
    try:
        ...
        # 6. Cluster unmapped objectives locally using Agglomerative Clustering
        draft_themes = []
        if unmapped_indices:
            logger.info("Clustering unmapped objectives locally...")
            unmapped_embeddings = objectives_embeddings[unmapped_indices]
            
            # Using cosine distance = 1 - cosine similarity
            # If distance < 0.45, it means cosine similarity >= 0.55
            ...
            # Fit clustering
            labels = clustering.fit_predict(unmapped_embeddings)
            unique_labels = np.unique(labels)
            logger.info(f"Identified {len(unique_labels)} new candidate clusters.")

            # Create placeholder themes for each cluster (LLM generation code is commented out)
            ...
            for label in unique_labels:
                ...
                # We can calculate average similarity within the cluster as the confidence score
                # Set a default baseline score for local clustering accuracy
                score = 0.85
